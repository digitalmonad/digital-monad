...
import { getSortedWork } from "@/lib/source/work";
...
export const Links = () => {
  ...
  const lists: ListItem[] = [
    ...
    },
    {
      items: [
        {
          children: "RSS Feed",
          href: `${siteConfig.url}${siteConfig.links.rss}`,
        },
        {
          children: "Sitemap",
          href: `${siteConfig.url}${siteConfig.links.sitemap}`,
        },
      ],
      title: "More",
    },
  ...
  return (
    <div className="grid grid-cols-2 gap-6 text-muted-foreground text-sm sm:grid-cols-4 sm:gap-8">
      {lists.map((list) => (
        <div className="flex flex-col gap-3 sm:gap-6" key={list.title}>
